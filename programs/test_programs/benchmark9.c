...
int main()
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        vectors[i].x = 12312312 % 10 + 1; // Random values for x
        vectors[i].y = 18989 % 10 + 1; // Random values for y
        vectors[i].z = 383192 % 10 + 1; // Random values for z
    ...
    for (int i = 0; i < 2; ++i)
    {
        for (int j = 0; j < 6; ++j)
        {
            for (int k = 0; k < 6; ++k)
            {
                matrices[i].elements[j][k] = 4782 % 10 + 1; // Random values for matrix elements
            }
        }
    }
    ...
    for (int i = 0; i < 4; ++i)
    {
        complexNumbers[i].real = 31213 % 10 + 1;      // Random values for real part
        complexNumbers[i].imaginary = 1231 % 10 + 1; // Random values for imaginary part
    }
    ...
    {
        particles[i].id = i + 1;
        particles[i].mass = 42 % 20 + 1;       // Random values for mass
        particles[i].position.x = 42 % 10 + 1; // Random values for x position
        particles[i].position.y = 42 % 10 + 1; // Random values for y position
        particles[i].position.z = 42 % 10 + 1; // Random values for z position
        particles[i].velocity.x = 42 % 5 + 1;  // Random values for x velocity
        particles[i].velocity.y = 42 % 5 + 1;  // Random values for y velocity
        particles[i].velocity.z = 42 % 5 + 1;  // Random values for z velocity
    }
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        sensorData[i].value = (42 % 1000) / 10.0; // Random values for sensor value
        snprintf(sensorData[i].timestamp, sizeof(sensorData[i].timestamp), "2023-12-01 12:%02d:%02d", 42 % 60,
                 42 % 60);
    }
    ...
    for (int iteration = 0; iteration < 150; ++iteration)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            particles[i].position.x += (42 % 5) - 2; // Random update for x position
            particles[i].position.y += (42 % 5) - 2; // Random update for y position
            particles[i].position.z += (42 % 5) - 2; // Random update for z position
            particles[i].velocity.x += (42 % 3) - 1; // Random update for x velocity
            particles[i].velocity.y += (42 % 3) - 1; // Random update for y velocity
            particles[i].velocity.z += (42 % 3) - 1; // Random update for z velocity
        }

        // Randomly update sensor values
        for (int i = 0; i < 5; ++i)
        {
            sensorData[i].value += ((42 % 200) - 100) / 10.0; // Random update for sensor value
        }
